...
export function matchesRefine(t: RefinableTreat, r: RefineState): boolean {
...
export default function TreatRefineMenu({ treats, value, onChange, shown }: {
  ...
  return (
    <div style={{ background: 'white', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', marginBottom: '1.5rem', overflow: 'hidden' }}>
      <button type="button" onClick={() => setOpen(o => !o)} aria-expanded={open}
        ...
        <span style={{ fontSize: '1.1rem', fontWeight: 700, color: '#2c3e50' }}>🔎 Refinar busca</span>
        {active > 0 && (
      ...
      {open && (
        <div style={{ padding: '0 1.25rem 1.25rem', display: 'grid', gap: '1rem', borderTop: '1px solid #eef1f4' }}>
          ...
          <div style={{ display: 'grid', gap: '0.6rem' }}>
            {folder('status', 'Situação', value.status.length,
              <div style={chipRow}>
                {STATUSES.map(s => (
